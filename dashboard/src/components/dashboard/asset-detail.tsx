...
const AssetMap = dynamic(
  () => import("./asset-map").then((mod) => mod.AssetMap),
  { ssr: false },
);
...
export function AssetDetail({
  ...
  const mediaById = Object.fromEntries(
    media.map((item) => [item.id, item.url]),
  );
    ...
    <div className="space-y-6">
      <section className="panel grid gap-6 p-6 lg:grid-cols-[1fr_360px]">
        <div>
          ...
          <h1 className="mt-3 text-[38px] font-semibold leading-[46px] text-primary">
            Asset {asset.qrCode}
          </h1>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <StatusBadge status={asset.status} />
            <span className="text-sm font-semibold text-on-surface-variant">
              Versao {asset.version}
            </span>
            <span className="text-sm font-semibold text-on-surface-variant">
              Tecnico: {asset.createdBy.name}
            </span>
          </div>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <Panel title="Manejos">
          ...
          {manejos.length === 0 ? (
            <Empty text="Sem manejos vinculados." />
          ) : null}
        </Panel>
        <Panel title="Monitoramentos">
          ...
          {monitoramentos.length === 0 ? (
            <Empty text="Sem monitoramentos vinculados." />
          ) : null}
        </Panel>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        ...
        >
          {history.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between rounded-[20px] bg-white/45 p-4"
            >
          ...
          {auditLogs.map((log) => (
            <div key={log.id} className="rounded-[20px] bg-white/45 p-4">
              ...
              <p className="mt-1 text-xs text-on-surface-variant">
                {log.performedBy.name || log.performedBy.id} -{" "}
                {new Date(log.createdAt).toLocaleString("pt-BR")}
              </p>
            </div>
          ))}
          {auditLogs.length === 0 ? (
            <Empty text="Sem eventos de auditoria visiveis." />
          ) : null}
        </Panel>
      </section>
